...
pub async fn main(rpc_client: Arc<RpcClient>) -> Result<()> {
    let payer = Arc::new(Keypair::new());
    println!("Payer: {}", payer.pubkey());
    airdrop(&rpc_client, &payer, 5).await?;
    ...
    println!("Recipient: {}", recipient_owner.pubkey());
    airdrop(&rpc_client, &recipient_owner, 1).await?;
...
async fn deposit_tokens(
    ...
    println!(
        "Depositing {} tokens to confidential pending balance...",
        amount / 10u64.pow(DECIMALS as u32)
    );
...
async fn transfer_tokens(
    ...
    println!(
        "Performing confidential transfer of {} tokens from {} to {}...",
        amount / 10u64.pow(DECIMALS as u32),
